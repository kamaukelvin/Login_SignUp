  ...
  actions: {
    //   hit the sign in endpoint and receive a token
      async signIn({commit},credentials){
        ...
        const response = await axios.post('api/login.php',credentials)
        // pass the token received to the validate token method using dispatch
        // return  dispatch('validateToken', response.data.jwt)
       
        commit('SET_USER', response.config.data)
    ...
    // async validateToken({commit, state}, token){
        ...
        //  try{
        //      const response= await axios.post('auth/validate_token.php')
    ...
    async register({commit},user){
        const response = await axios.post('api/create_user.php',user)
        commit('SET_USER', response.config.data)
